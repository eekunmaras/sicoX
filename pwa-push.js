...
async function savePushSubscription(subscription, prefs = {}) {
  ...
  const record = {
    ...
    // DB側の notify_* は「push配送を受けるか」。
    // 実際にバナーを出すかは SW に同期した prefs.notifyTweet/Dm/Comment が決める。
    // そのため「バナーOFF + バッジON」では push だけ受け取り、SW側でバナーを抑止してバッジだけ付ける。
...
async function updatePushPref(key, value) {
  ...
  // DB の設定も更新する。
  // すでに通知許可済みなのに購読が未作成の場合は、ここで購読作成まで進める。
  const reg = await navigator.serviceWorker.ready;
